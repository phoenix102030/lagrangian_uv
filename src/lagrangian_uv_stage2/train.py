...
def _compute_batch_loss_and_metrics(
    ...
    if "kernel_mix" in outputs:
        metrics["kernel_mix"] = outputs["kernel_mix"].mean()
    if "persistence_diagonal" in outputs:
        metrics["persistence_mean"] = outputs["persistence_diagonal"].mean()
    return loss, metrics
